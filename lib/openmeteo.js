...
const windLevelStr = (kmh) => Math.round(kmh / 3.6) + " м/с";
...
const cityId = (city) => city.id || city.areaId;
...
async function forecast(city) {
  ...
  return daily.time.map((date, i) => {
    ...
    return {
      areaId: cityId(city),
      updateTime: date,
      allMaxTemp: String(Math.round(daily.temperature_2m_max[i])),
      allMinTemp: String(Math.round(daily.temperature_2m_min[i])),
      dayWeathPheno: pheno,
