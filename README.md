...
Список городов хранится на устройстве. `/climate/cityList` получает локальный
`CityOrderBean` из query-параметра `citys` и только добавляет актуальную погоду;
`/climate/bindcity` намеренно является no-op. Поэтому добавленные и удалённые
города не теряются при холодном старте Vercel.

Для совместимости со старыми чистыми установками proxy считает legacy-ID
`110105` синонимом Минска (`BY_01`).
