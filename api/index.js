// Vercel serverless handler — Geely weather proxy with Belarusian cities.
// Routes /geely/znzc/oneos/climate/* : BY cities → Open-Meteo, rest → Geely cloud.
'use strict';

const { BY_CITIES, byId, cityBean, nearestBYCity, isInBelarus } = require('../lib/cities');
...
async function proxyGeely(req, path) {
  ...
  for (const [k, v] of Object.entries(req.headers)) {
    if (!['host', 'connection', 'x-forwarded-for', 'x-forwarded-proto', 'x-vercel-proxy'].includes(k.toLowerCase())) {
      headers[k] = v;
    }
  }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (path.endsWith('/climate/current')) {
      if (isBY && city) return res.status(200).json(ok(await openmeteo.current(city)));
      const r = await proxyGeely(req, req.url);
      return res.status(r.status).send(r.body);
    }
    ...
    if (path.endsWith('/climate/hours')) {
      if (isBY && city) return res.status(200).json(ok(await openmeteo.hours(city)));
      const r = await proxyGeely(req, req.url);
      return res.status(r.status).send(r.body);
    }

    // --- 15-day forecast ---
    if (path.endsWith('/climate/newhalfmonth') || path.endsWith('/climate/halfmonth')) {
      if (isBY && city) return res.status(200).json(ok(await openmeteo.forecast(city)));
      const r = await proxyGeely(req, req.url);
      return res.status(r.status).send(r.body);
    }
    ...
    if (path.endsWith('/climate/sunriseAndSunset')) {
      if (isBY && city) return res.status(200).json(ok(await openmeteo.sun(city)));
      const r = await proxyGeely(req, req.url);
      return res.status(r.status).send(r.body);
    }
    ...
    res.status(500).json({ code: 500, message: err.message, success: false, data: null });
  }
};
